...
sudo action refreshes the credential before running Homebrew's upstream installer
as the regular user, noninteractively. `--allow-sudo` authorizes that one action;
it can be omitted when Homebrew is already installed. On a
